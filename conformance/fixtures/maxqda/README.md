# MAXQDA export fixtures

## What to drop here

`.qdpx` files produced by **importing `conformance/fixtures/seed/seed.qdpx`
into MAXQDA and re-exporting it as REFI-QDA**. This is the canonical path
(see `conformance/README.md`) -- it isolates MAXQDA's exporter behaviour
from operator variance, because the input is the same file every time,
not a project built by hand a second time in MAXQDA's UI.

Name each file to record the MAXQDA version that produced it, e.g.:

```
maxqda_2022_export.qdpx
maxqda_2024_export.qdpx
```

## Also acceptable

A project built directly in MAXQDA by hand (rather than imported from the
...
1. Parses it with `refi_qda.parser.parse_qdpx` (fails loudly if MAXQDA's
